...
class ChunkManager:
    def __init__(self):
        ...
        self.pending_chunk_changes: dict[int, list[tuple]] = {}

        self.save = save_manager.SaveManager(self)
        self.tree_generator = TreeGenerator()

        # 之後加"grass_color": (70,180,70),
        self._BIOMES: dict[str, dict[str, int | float | str | dict[str, float] | None]] = {
            "plains": {
                "temp": -0.8,
                "humidity": 0.0,
                "surface": "grass",
    ...
    def set_block(self, world_x, world_y, block_type):
        ...
        chunk = self.get_chunk(chunk_i)

        chunk.blocks[chunk_y][local_x] = block_type
        chunk.is_dirty = True

    def get_chunk(self, chunk_x, fluid_manager: FluidManager = None) -> Chunk:
        # 1. 已經載入
        if chunk_x in self.chunks:
            return self.chunks[chunk_x]

        # 2. 嘗試讀取存檔
        loaded_chunk = self.save.load_chunk(chunk_x)
        world_x = chunk_x * config.CHUNK_WIDTH

        if loaded_chunk is not None:
            biome_name = self.get_biome(world_x)

            self.chunks[chunk_x] = Chunk(chunk_x, loaded_chunk, biome_name)
            if fluid_manager is not None:
                fluid_manager.register_chunk_fluids(chunk_x, self.chunks[chunk_x])
            return self.chunks[chunk_x]

        # 3. 沒有存檔就生成
        blocks, biome_name = self.make_map(config.CHUNK_WIDTH, config.MAP_HEIGHT, chunk_x)
    ...
    def make_map(self, map_width, map_height, current_chunk_i):
        ...
        height_map = self._make_terrain(current_chunk_i)
        terrain_time = time.perf_counter() - start

        start = time.perf_counter()
        chunk_data = self._make_base_terrain(
            map_width,
            map_height,
            current_chunk_i,
            biome_name,
        ...
        start = time.perf_counter()
        chunk_data = self._generate_caves(
            current_chunk_i,
            chunk_data,
            height_map,
        )
        ...
        start = time.perf_counter()
        chunk_data = self._generate_cave_entrances(
            current_chunk_i,
            chunk_data,
            height_map,
            rng,
        ...
        start = time.perf_counter()
        chunk_data = self._generate_trees(
            current_chunk_i,
            self._BIOMES[biome_name],
            chunk_data,
            height_map,
            rng,
        )

        if current_chunk_i in self.pending_chunk_changes:
            for wx, wy, block_name in self.pending_chunk_changes[current_chunk_i]:
                local_x = wx - (current_chunk_i * config.CHUNK_WIDTH)
                if 0 <= wy < config.MAP_HEIGHT:
                    chunk_data[wy][local_x] = block_name

            # 應用完後清空暫存
            del self.pending_chunk_changes[current_chunk_i]

        trees_time = time.perf_counter() - start

        start = time.perf_counter()
        chunk_data = self._cleanup_terrain(
            current_chunk_i,
            chunk_data,
            height_map,
        )
        ...
        start = time.perf_counter()
        chunk_data = self._generate_underground_fluids(
            current_chunk_i,
            chunk_data,
            height_map,
        )
        ...
        start = time.perf_counter()
        chunk_data = self._generate_veins(
            current_chunk_i,
            chunk_data,
            rng,
        )
    ...
    def _get_uniform_noise(self, world_x: float, seed: int) -> float:
    ...
    def get_biome(self, world_x: int) -> str:
        # 1. 取得校正後的溫度與濕度 (1000, 2000 為不同的種子碼)
        temp_noise = self._get_uniform_noise(world_x, 1000)
        humidity_noise = self._get_uniform_noise(world_x, 2000)

        best_biome = "plains"
        min_distance = float("inf")

        # 2. 尋找與當前氣候最接近的生態系
        for name, data in self._BIOMES.items():
    ...
    def _make_terrain(self, chunk_x):
        self.height_map = []
        ...
        for local_x in range(config.CHUNK_WIDTH):
            ...
            self.height_map.append(current_height)

        return self.height_map

    def _make_base_terrain(self, map_width, map_height, chunk_x, biome_name, height_map, rng: random.Random):
        chunk_data = [["air"] * map_width for _ in range(map_height)]
        biome = self._BIOMES[biome_name]
    ...
    def _generate_caves(self, chunk_x, chunk_data, height_map):
    ...
    def _generate_cave_entrances(self, chunk_x, chunk_data, height_map, rng: random.Random):
        for local_x in range(config.CHUNK_WIDTH):
    ...
    def _cleanup_terrain(self, chunk_x, chunk_data, height_map):
    ...
    def _generate_underground_fluids(self, chunk_x, chunk_data, height_map):
        base_world_x = chunk_x * config.CHUNK_WIDTH
    ...
    def _generate_veins(self, chunk_x, chunk_data, rng: random.Random):
        ore_rules = [
            # =========================
            # Stone
            # =========================
        ...
        for rule in ore_rules:
            ...
            for y in range(config.MAP_HEIGHT):
                for local_x in range(config.CHUNK_WIDTH):
                    ...
                    self._spawn_vein(chunk_data, vein_size, y, local_x, rule["name"], rng)

        return chunk_data

    def _spawn_vein(self, chunk_data, vein_size, center_y, center_x, vein_name, rng: random.Random):
        blocks_placed = 0
    ...
    def _generate_trees(self, chunk_x: int, biome: dict, chunk_data: list, height_map: list, rng: random.Random):
        """
        ...
        for _ in range(tree_count):
            for _ in range(10):  # 最多嘗試 10 次找合適位置
                ...
                if not self.tree_generator._can_place_tree(plant_world_x, placed_tree_x, tree_spawn_CD):
                    continue

                if self._has_nearby_log(plant_world_x, tree_spawn_CD):
                    continue
                ...
                tree_blocks = self.tree_generator.generate(self._choose_tree_type(biome, rng), plant_world_x, surface_y, rng)
                ...
                for wx, wy, block_name in tree_blocks:
                    ...
                    elif target_chunk_x in self.chunks:
                        self.set_block(wx, wy, block_name)
                    else:
                        if target_chunk_x not in self.pending_chunk_changes:
                            self.pending_chunk_changes[target_chunk_x] = []
                        self.pending_chunk_changes[target_chunk_x].append((wx, wy, block_name))

                break

        return chunk_data

    def _has_nearby_log(self, candidate_world_x, radius):
        for wx in range(candidate_world_x - radius, candidate_world_x + radius + 1):
            ...
            # 情況1：鄰居已經生成過 -> 直接讀 self.chunks，絕對不要呼叫 get_block/get_chunk（會觸發生成）
            if chunk_x in self.chunks:
                for y in range(config.MAP_HEIGHT):
                    if self.chunks[chunk_x].blocks[y][local_x].endswith("_log"):
                        return True

            # 情況2：鄰居還沒生成，但已經有排隊要塞的方塊（可能是隔壁樹冠延伸過來的）
            if chunk_x in self.pending_chunk_changes:
                for pwx, _, block_name in self.pending_chunk_changes[chunk_x]:
                    if pwx == wx and block_name.endswith("_log"):
                        return True

        return False

    def _choose_tree_type(self, biome, rng: random.Random):
        tree_type = biome.get("tree")
    ...
    def find_spawn_position(self, spawn_x):
        pass
